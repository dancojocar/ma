...
public class ListFlipper extends Activity {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    // Prepare the ListView
    final ArrayAdapter<String> adapterRo = new ArrayAdapter<>(this,
        android.R.layout.simple_list_item_1, LIST_STRINGS_RO);

    mEnglishList.setAdapter(adapterEn);
    mRomanianList.setAdapter(adapterRo);
  ...
  private void flip() {
    ...
    ObjectAnimator visibleToInvisible =
        ObjectAnimator.ofFloat(visibleList,
            "rotationY", 0f, 90f);
    visibleToInvisible.setDuration(DURATION);
    visibleToInvisible.setInterpolator(accelerator);

    final ObjectAnimator invisibleToVisible =
        ObjectAnimator.ofFloat(invisibleList,
            "rotationY", -90f, 0f);
