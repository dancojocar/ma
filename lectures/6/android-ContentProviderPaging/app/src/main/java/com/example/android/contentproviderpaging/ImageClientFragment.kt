...
class ImageClientFragment : Fragment() {
  ...
  override fun onViewCreated(rootView: View, savedInstanceState: Bundle?) {
    ...
    recyclerView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
      override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
        ...
        if (lastVisiblePosition >= mAdapter!!.fetchedItemCount) {
          logd("Fetch new images. LastVisiblePosition: " + lastVisiblePosition
          ...
          activity.supportLoaderManager.restartLoader(pageId, null, mLoaderCallback)
        }
      }
    })
    ...
    showButton.setOnClickListener {
      activity.supportLoaderManager.restartLoader(0, null, mLoaderCallback)
      ...
      return object : CursorLoader(activity) {
        override fun loadInBackground(): Cursor? {
          ...
          return activity.contentResolver
              .query(ImageContract.CONTENT_URI,
                  null, bundle, null)
        }
      }
    }

    override fun onLoadFinished(loader: Loader<Cursor>, cursorArg: Cursor?) {
      val cursor = cursorArg!!
