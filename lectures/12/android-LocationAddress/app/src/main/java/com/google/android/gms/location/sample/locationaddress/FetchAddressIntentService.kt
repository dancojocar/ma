...
class FetchAddressIntentService : IntentService("FetchAddress") {
  ...
  override fun onHandleIntent(intent: Intent?) {
    ...
    if (intent == null || receiver == null) {
      loge("No receiver received. There is nowhere to send the results.")
      return
    }
    ...
    try {
      ...
      loge(errorMessage, ioException)
    } catch (illegalArgumentException: IllegalArgumentException) {
      // Catch invalid latitude or longitude values.
      errorMessage = getString(R.string.invalid_lat_long_used)
      loge("$errorMessage. Latitude = $location.latitude , " +
    ...
    if (addresses.isEmpty()) {
      ...
      logi(getString(R.string.address_found))
