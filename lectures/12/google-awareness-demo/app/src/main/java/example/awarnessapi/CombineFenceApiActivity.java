...
public class CombineFenceApiActivity extends AppCompatActivity implements View.OnClickListener,
  ...
  private static final String COMBINE_FENCE_ENTERING_KEY = "enteringCombineFence";
  ...
  private void registerFence() {
    ...
    AwarenessFence locationFence = LocationFence.in(
        46.7677992,    //Latitude of place
        23.5890875,    //Longitude of place
        50.00,         //Radius in meters
        5 * 1000);     //Wait for five seconds
    ...
    long startTime = 10 * 60 * 60 * 1000;
    long stopTime = 12 * 60 * 60 * 1000;

    ArrayList<AwarenessFence> timeFences = new ArrayList<>(6);
    timeFences.add(TimeFence.inMondayInterval(TimeZone.getDefault(), startTime, stopTime));
    timeFences.add(TimeFence.inTuesdayInterval(TimeZone.getDefault(), startTime, stopTime));
    timeFences.add(TimeFence.inWednesdayInterval(TimeZone.getDefault(), startTime, stopTime));
    timeFences.add(TimeFence.inThursdayInterval(TimeZone.getDefault(), startTime, stopTime));
    timeFences.add(TimeFence.inFridayInterval(TimeZone.getDefault(), startTime, stopTime));
    AwarenessFence timesFences = AwarenessFence.or(timeFences);

    /**
     * Now apply and fence to location fence and OR-ED time fences.
     */
    AwarenessFence andFence = AwarenessFence.and(locationFence, timesFences);
      ...
      if (TextUtils.equals(fenceState.getFenceKey(), COMBINE_FENCE_ENTERING_KEY)) {
        switch (fenceState.getCurrentState()) {
          case FenceState.TRUE:
            mStatusTv.setText("You are at UBB.");
            AudioManager audio = (AudioManager) getSystemService(Context.AUDIO_SERVICE);
            audio.setRingerMode(AudioManager.RINGER_MODE_SILENT);
            break;
          case FenceState.FALSE:
            mStatusTv.setText("You are not at UBB.");
            audio = (AudioManager) getSystemService(Context.AUDIO_SERVICE);
            audio.setRingerMode(AudioManager.RINGER_MODE_NORMAL);
            break;
          case FenceState.UNKNOWN:
            mStatusTv.setText("A little bit confused.:(");
            break;
        }
      }
    }
