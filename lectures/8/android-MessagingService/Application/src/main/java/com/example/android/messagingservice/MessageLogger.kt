...
internal object MessageLogger {
  ...
  private val PREF_MESSAGE = "MESSAGE_LOGGER"
  private val DATE_FORMAT = SimpleDateFormat.getDateTimeInstance()
  private val LINE_BREAKS = "\n\n"

  val LOG_KEY = "message_data"

  fun logMessage(context: Context?, message: String) {
    val prefs = getPrefs(context)
    val messageString = "${DATE_FORMAT.format(Date(System.currentTimeMillis()))}: $message"
    prefs!!.edit()
        .putString(LOG_KEY, "${prefs.getString(LOG_KEY, "")}$LINE_BREAKS$messageString")
        .apply()
  }
