...
public class BillingUnitTest {
    ...
    public void billingHelperData_isConsistent() throws Exception {
        List<String> inAppList = BillingConstants.INSTANCE.getSkuList(BillingClient.SkuType.INAPP);
        assertTrue(inAppList.contains(INSTANCE.getSKU_PREMIUM()));
        assertTrue(inAppList.contains(INSTANCE.getSKU_GAS()));
        List<String> subscriptionsList = BillingConstants.INSTANCE.getSkuList(BillingClient.SkuType.SUBS);
        assertTrue(subscriptionsList.contains(INSTANCE.getSKU_GOLD_MONTHLY()));
        assertTrue(subscriptionsList.contains(INSTANCE.getSKU_GOLD_YEARLY()));
    }
}
