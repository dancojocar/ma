...
import co.temy.securitysample.logd
...
class SafetyNet(private val context: Context) {
  ...
  fun printApiKey() = logd("Android API KEY: ${Utils.getSigningKeyFingerprint(context)}; ${context.packageName}")
  ...
  private fun goCheckWithLibrary() {
    ...
    safetyNetHelper.requestTest(context, object : SafetyNetHelper.SafetyNetWrapperCallback {
      ...
      override fun success(ctsProfileMatch: Boolean, basicIntegrity: Boolean) {
        logd("SafetyNet req success: ctsProfileMatch:$ctsProfileMatch and basicIntegrity, $basicIntegrity")
  ...
  private fun handleError(errorCode: Int, errorMessage: String) {
    logd("Something went wrong: $errorCode")
  ...
  private fun showWarningAlert(message: String, negativeCallback: (() -> Unit)?) {
    val builder = AlertDialog.Builder(context)
        ...
        .setPositiveButton(R.string.exit_btn) { _, _ -> exitProcess(0) }
        .setMessage(message)
        .setCancelable(false)
